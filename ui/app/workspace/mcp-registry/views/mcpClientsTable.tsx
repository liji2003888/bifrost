...
export default function MCPClientsTable({
	...
	const getToolSnapshotDescription = (client: MCPClient) => {
		switch (client.tool_snapshot_source) {
			case "persisted":
				return "Using ConfigStore snapshot";
			case "live":
				return "Live tool discovery";
			default:
				return "";
		}
	};
					...
					<TableBody>
						{mcpClients.length === 0 ? (
							...
							mcpClients.map((c: MCPClient) => {
								const discoveredToolCount = c.tools?.length ?? 0;
								...
								return (
									...
									>
										...
										<TableCell>
											{discoveredToolCount > 0 ? (
												<div className="flex flex-col gap-0.5">
													<span>
														{enabledToolsCount}/{discoveredToolCount}
													</span>
													{getToolSnapshotDescription(c) ? (
														<span className="text-muted-foreground text-xs">{getToolSnapshotDescription(c)}</span>
													) : null}
												</div>
											) : (
												"-"
										...
										<TableCell>
											{discoveredToolCount > 0 ? (
												<>
													{autoExecuteToolsCount}/{discoveredToolCount}
												</>
											) : (
												"-"
											)}
